package com.commusoft.diary.diarytrials.DailyView;
...
public class DailyView extends View {
    ...
    private class EventRect {
        public DailyViewEvent event;
        public DailyViewEvent originalEvent;
        public RectF rectF;
        public float left;
        public float width;
        public float top;
         ...
         */
        public EventRect(DailyViewEvent event, DailyViewEvent originalEvent, RectF rectF) {
            this.event = event;
            this.rectF = rectF;
    ...
    private void getMoreEvents(Calendar day) {
        ...
        if (mFetchedMonths[0] < 1 || mFetchedMonths[0] != previousMonth || mRefreshEvents) {
            if (!containsValue(lastFetchedMonth, previousMonth) && !isInEditMode()){
                List<DailyViewEvent> events = mMonthChangeListener.onMonthChange((previousMonth==12)?day.get(Calendar.YEAR)-1:day.get(Calendar.YEAR), previousMonth);
                sortEvents(events);
                for (DailyViewEvent event: events) {
                    cacheEvent(event);
                }
            }
        ...
        if (mFetchedMonths[1] < 1 || mFetchedMonths[1] != day.get(Calendar.MONTH)+1 || mRefreshEvents) {
            if (!containsValue(lastFetchedMonth, day.get(Calendar.MONTH)+1) && !isInEditMode()) {
                List<DailyViewEvent> events = mMonthChangeListener.onMonthChange(day.get(Calendar.YEAR), day.get(Calendar.MONTH) + 1);
                sortEvents(events);
                for (DailyViewEvent event : events) {
                    cacheEvent(event);
                }
        ...
        if (mFetchedMonths[2] < 1 || mFetchedMonths[2] != nextMonth || mRefreshEvents) {
            if (!containsValue(lastFetchedMonth, nextMonth) && !isInEditMode()) {
                List<DailyViewEvent> events = mMonthChangeListener.onMonthChange(nextMonth == 1 ? day.get(Calendar.YEAR) + 1 : day.get(Calendar.YEAR), nextMonth);
                sortEvents(events);
                for (DailyViewEvent event : events) {
                    cacheEvent(event);
                }
     ...
     */
    private void cacheEvent(DailyViewEvent event) {
        if (!isSameDay(event.getStartTime(), event.getEndTime())) {
            ...
            DailyViewEvent event1 = new DailyViewEvent(event.getId(), event.getName(), event.getStartTime(), endTime);
            event1.setColor(event.getColor());
            DailyViewEvent event2 = new DailyViewEvent(event.getId(), event.getName(), startTime, event.getEndTime());
     ...
     */
    private void sortEvents(List<DailyViewEvent> events) {
        Collections.sort(events, new Comparator<DailyViewEvent>() {
            @Override
            public int compare(DailyViewEvent event1, DailyViewEvent event2) {
     ...
     */
    private boolean isEventsCollide(DailyViewEvent event1, DailyViewEvent event2) {
        long start1 = event1.getStartTime().getTimeInMillis();
        long end1 = event1.getEndTime().getTimeInMillis();
        long start2 = event2.getStartTime().getTimeInMillis();
        long end2 = event2.getEndTime().getTimeInMillis();
